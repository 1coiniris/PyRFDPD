...
for model in model_map:
    ...
    if Model == 'GMP_NN':
        ...
        Lc = model[1]
        M_len = (M + 1 + Lb + Lc)
        input_size = (M + 1 + Lb + Lc) # 输入维度
        output_size = 2 * (M + 1)  # 输出维度
        # output_size = 2
        hidden_size = [model[2],model[3],model[4]]  # 隐藏层维度
        # hidden_layer = 5
        ...
        if model_train == 1:
            ...
            for epoch in range(epochs):
                ...
                for inputs, targets in tqdm(train_loader):
                    ...
                    reshaped_tensor = inputs.view(len(inputs), input_size, 2)
                    ...
                    # 将数据移动到设备上
                    batch_x_magnitude = abs(input_tensor).to(device)
