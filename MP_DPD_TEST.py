...
plt.ylim(0,1)
plt.legend()
plt.savefig('figures/MP/PA_waveform.png')
# plt.show()
# sleep(5)
plt.close()
...
plot.psd(
    ...
    fs=fs,filename='figures/MP/PA_Spectrum.png'
)
# a = list(xorg)
# TEST_Plot.plot_power_spectrum({"PA input": x,"PA output":y},100e6)
# TEST_Plot.plot_amam(x, y,filename="figures/amam wo DPD.png")
plot.amam(x, {"wo DPD":y}, "figures/MP/PA_amam.png")

PA_in = xorg
PA_out = yorg
coef = mp.MP_e(PA_out, PA_in, 5, 7, N)
print(coef)
DPD = mp.MP_v(PA_in, coef, 5, 7)
DPD = DPD.reshape(-1,1)
...
plot.psd(
    ...
    fs=fs,filename='figures/MP/MP_DPD_spec.png'
)
plot.amam(x, {"wo DPD":y,"DPD":DPD,"with DPD":PA_out_withDPD}, "figures/MP/MP_DPD_amam.png")


plt.figure()
t = np.linspace(0, 1, 200)
plt.plot(t,abs(PA_out_withDPD[2000:2200]),label = 'PA_Output')
plt.plot(t,abs(x[2000:2200]),label = 'PA_Input')
plt.plot(t,abs(DPD[2000:2200]),label = 'DPD')
# plt.xlim(0,200)
plt.ylim(0,1)
plt.legend()
plt.savefig('figures/MP/DPD_waveform.png')
