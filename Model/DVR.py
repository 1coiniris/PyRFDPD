...
class DVR_NN(nn.Module):
    ...
    def DVR_NN_e(self,x,y):
        ...
        x_window = torch.from_numpy(sequences)

        X = self.get_basis(x_window).detach().numpy()
        XH = np.conjugate(X.T)
        coef = np.linalg.pinv(XH.dot(X) + 0.000001 * np.eye(X.shape[1])).dot(XH).dot(y)
        return coef
