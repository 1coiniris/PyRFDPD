...
def calculate_metrics(args: argparse.Namespace, stat: Dict[str, Any], prediction: np.ndarray, ground_truth: np.ndarray):
    stat['NMSE'] = NMSE(prediction, ground_truth)
    stat['EVM'] = EVM(prediction, ground_truth, bw_main_ch=args.bw_main_ch, n_sub_ch=args.n_sub_ch, nperseg=args.nperseg)
    ACLR_L = []
    ACLR_R = []
    ACLR_left, ACLR_right = ACLR(prediction, fs=args.input_signal_fs, nperseg=args.nperseg,
