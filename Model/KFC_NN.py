...
class KFC_NN(nn.Module):
    def __init__(self, K, M, M2,  activation="ReLU"):
        ...
        for index, (in_dim, out_dim) in enumerate(zip(layer_dims_phase[:-1], layer_dims_phase[1:])):
            self.phase_layers.add_module("linear " + str(index), nn.Linear(in_dim, out_dim).double())
            if activation == "ReLU":
                self.phase_layers.add_module("actFunc " + str(index), nn.ReLU())
            elif activation == "Tanh":
                self.phase_layers.add_module("actFunc " + str(index), nn.Tanh())
            elif activation == "GELU":
                self.phase_layers.add_module("actFunc " + str(index), nn.GELU())
        if self.activation != "ABS":
    ...
    def get_basis(self,x_window):
        ...
        # x(n-i) (16384,M2+1)
        X_tensor = x_window #记忆信号x(n-i) (16384,M2+1)
        ...
        # |x(n-i)| DVR CORE
        X_amp = torch.abs(X_tensor)  #|x(n-i)| (16384,M2+1)
        ...
        # theta(n-i) DVR PHASE
        X_phase = torch.angle(X_tensor_M)  #theta(n-i) (16384,M+1)
    ...
    def create_dataset(self,x, y, test_size=0.2):
        ...
        sequences = volterra_nn.create_memory_seq(x,M2)  # [N, M+1]
    ...
    def apply_dpd(self, signal,coef = None):
        ...
        self.to(device)
        self.eval()
        sequences = volterra_nn.create_memory_seq(signal, M2)  # [N, M+1]
